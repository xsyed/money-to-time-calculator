...
(function(){
  ...
  const DAYS_KEY = 'mtt_business_days'
  const HOURS_KEY = 'mtt_business_hours'

  // Elements
  const annualDisplay = document.getElementById('annual-display')
  const businessDaysDisplay = document.getElementById('business-days-display')
  ...
  const businessDaysInput = document.getElementById('business-days-input')
  const businessHoursInput = document.getElementById('business-hours-input')
  ...
  function loadBusinessDays(){
    const raw = localStorage.getItem(DAYS_KEY)
    if (!raw) return 5 // Default to 5 business days
    const v = Number(raw)
    return isFinite(v) && v >= 1 && v <= 7 ? v : 5
  }

  function saveBusinessDays(v){
    if (v == null || !isFinite(v) || v < 1 || v > 7) return
    localStorage.setItem(DAYS_KEY, String(v))
  }

  function loadBusinessHours(){
    const raw = localStorage.getItem(HOURS_KEY)
    if (!raw) return 8 // Default to 8 business hours
    const v = Number(raw)
    return isFinite(v) && v >= 1 && v <= 24 ? v : 8
  }

  function saveBusinessHours(v){
    if (v == null || !isFinite(v) || v < 1 || v > 24) return
    localStorage.setItem(HOURS_KEY, String(v))
  }
  ...
  function calcRates(annual){
    ...
    // Get business days and hours from localStorage
    const businessDays = loadBusinessDays()
    const businessHours = loadBusinessHours()
    
    // Calculate hourly rate based on 52 weeks per year and custom business hours/days per week
    // Using precise math to avoid floating-point errors
    const hourly = parseFloat((annual / (52 * businessDays * businessHours)).toFixed(4))
    return {monthly, biweekly, hourly, businessDays, businessHours}
  }
  ...
  function formatWorkTime(hours){
    ...
    // Get current business time units
    const businessDays = loadBusinessDays()
    const businessHours = loadBusinessHours()

    // Business time units based on user settings:
    const HOUR = 1
    const DAY = businessHours * HOUR
    const WEEK = businessDays * DAY
    const MONTH = 4 * WEEK
    const YEAR = 12 * MONTH
  ...
  function renderAnnual(){
    ...
    // Update business days and hours displays
    businessDaysDisplay.textContent = loadBusinessDays()
    businessHoursDisplay.textContent = loadBusinessHours()
  }
  ...
  function calculate(){
    ...
    const {hourly, businessDays, businessHours} = calcRates(annual)
    // Calculate total hours with higher precision
    const totalHours = parseFloat((purchase / hourly).toFixed(6))

    const formatted = formatWorkTime(totalHours)

    resultEl.innerHTML = `<strong>${formatted}</strong> — equivalent to ${moneyFmt(hourly)} / hour<br><small>(Based on ${businessDays} days/week, ${businessHours} hours/day)</small>` // eslint-disable-line
  }
  ...
  editBtn.addEventListener('click',()=>{
    ...
    businessDaysInput.value = String(loadBusinessDays())
    businessHoursInput.value = String(loadBusinessHours())
    showIncomeForm(true)
  })
  ...
  incomeForm.addEventListener('submit',(e)=>{
    ...
    // Get and validate business days input
    const businessDays = Number(businessDaysInput.value)
    if (!isFinite(businessDays) || businessDays < 1 || businessDays > 7){
      alert('Please enter a number between 1 and 7 for business days per week.')
      return
    }
    
    // Get and validate business hours input
    const businessHours = Number(businessHoursInput.value)
    if (!isFinite(businessHours) || businessHours < 1 || businessHours > 24){
      alert('Please enter a number between 1 and 24 for business hours per day.')
      return
    }
    
    // Save all values
    saveAnnual(v)
    saveBusinessDays(businessDays)
